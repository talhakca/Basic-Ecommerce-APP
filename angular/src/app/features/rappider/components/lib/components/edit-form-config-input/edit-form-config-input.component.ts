...
import { CheckboxComponentConfig } from '../../utils/checkbox';
...
export class RappiderEditFormConfigInputComponent
  implements OnInit, ControlValueAccessor
{
  ...
  getInputChangeReactions() {
    this.inputChangeReactions = Object.values(
  ...
  getSubmitButtonData() {
    ...
    this.submitButton = this.value?.submitButton
      ? this.value.submitButton
      : defaultSubmitButtonData;
  }
  ...
  getFormLayoutData() {
    this.formLayout = this.value?.formLayout
      ? this.value.formLayout
      : FormLayout.Vertical;
  }

  getInputChangeReactionData() {
    this.inputChangeReaction = this.value?.inputChangeReaction
      ? this.value.inputChangeReaction
      : CrudFormConfigInputChangeReaction.Default;
  }

  getItemSettingsData() {
    this.itemSettings = this.value?.itemSettings
      ? JSON.stringify(this.value.itemSettings)
      : '';
  }
  ...
  onItemSettingsChange() {
    try {
      ...
      this.notificationService.createNotification(
        'error',
        'SHARED.ERROR',
        'CRUD_VIEW_MODULE.JSON_ARRAY_COMPONENT.WRONG_JSON_FORMAT'
      );
    }
  }
